...
from utils_for_snn_lth_temp_lamp import *
...
def main():
    ...
    writer = SummaryWriter(f"./runs/lamp_VGGSNN_decision/{args.dataset}_{args.arch}")
    ...
    ckpt = torch.load(
        f"{os.getcwd()}/snn_laterewind_lth/lamp_VGGSNN_adam/{args.arch}/{args.dataset}/itearation0/ckpt.pth.tar")
    model.load_state_dict(ckpt['net'])
    functional.set_backend(model, 'cupy')
    initial_state_dict = copy.deepcopy(model.state_dict())
    timestep = find_decision(args, model, trainset=dataset_train)

    utils.checkdir(f"{os.getcwd()}/snn_laterewind_lth/lamp_VGGSNN_decision/{args.arch}/{args.dataset}/init_rewind")
    ...
    if args.resume_iter != 0:
        ckpt = torch.load(
            f"{os.getcwd()}/snn_laterewind_lth/lamp_VGGSNN_adam/{args.arch}/{args.dataset}/itearation{args.resume_iter - 1}/ckpt.pth.tar")
        model = VGGSNN(spiking_neuron=neuron.LIFNode, surrogate_function=surrogate.ATan(), detach_reset=True).cuda()
        model.load_state_dict(ckpt['net'])
        functional.set_backend(model, 'cupy')
        mask = ckpt['mask']
    ...
    for _ite in range(args.resume_iter, ITERATION):
        ...
        if args.scheduler is not None:
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=int(args.end_iter - rewinding_epoch),eta_min=0)
        ...
        utils.checkdir(
            f"{os.getcwd()}/dumps/snn_laterewind_lth/lamp_VGGSNN_decision/{args.arch}/{args.dataset}/itearation{_ite}")
        with open(
                f"{os.getcwd()}/dumps/snn_laterewind_lth/lamp_VGGSNN_decision/{args.arch}/{args.dataset}/itearation{_ite}/pruned{comp1}.pkl",
                'wb') as fp:
            pickle.dump(mask, fp)
        loss = 0
        accuracy = 0
        for iter_ in range(args.resume_epoch + 1, args.end_iter - rewinding_epoch + 1):
            ...
            if (iter_) % args.valid_freq == 0 or iter_ == 1:
                ...
                if accuracy > best_accuracy:
                    ...
                    utils.checkdir(
                        f"{os.getcwd()}/snn_laterewind_lth/lamp_VGGSNN_decision/{args.arch}/{args.dataset}/itearation{_ite}")
                    torch.save(model,
                               f"{os.getcwd()}/snn_laterewind_lth/lamp_VGGSNN_decision/{args.arch}/{args.dataset}/itearation{_ite}/pruned{1 - comp1}_acc{accuracy}_epoch{iter_ + rewinding_epoch}_model.pth.tar")
            ...
            if _ite == 0 and iter_ == args.rewinding_epoch:
                ...
                initial_state_dict = copy.deepcopy(model.state_dict())
                utils.checkdir(
                    f"{os.getcwd()}/snn_laterewind_lth/lamp_VGGSNN_decision/{args.arch}/{args.dataset}/init_rewind")
                torch.save(initial_state_dict,
                           f"{os.getcwd()}/snn_laterewind_lth/lamp_VGGSNN_decision/{args.arch}/{args.dataset}/init_rewind/rewind_state_dict.pth.tar")
        ...
        utils.checkdir(f"{os.getcwd()}/snn_laterewind_lth/lamp_VGGSNN_decision/{args.arch}/{args.dataset}/itearation{_ite}")
        torch.save(checkpoint,
                   f"{os.getcwd()}/snn_laterewind_lth/lamp_VGGSNN_decision/{args.arch}/{args.dataset}/itearation{_ite}/ckpt.pth.tar")
        best_accuracy = 0
    writer.close()
